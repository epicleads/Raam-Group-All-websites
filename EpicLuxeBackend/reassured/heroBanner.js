 ...
 */
router.post('/reassured-upload-hero', upload.fields([
  ...
  try {
    ...
    const { error: pcUploadError } = await supabase.storage
      .from(process.env.SUPABASE_BUCKET_HERO_REASSURED)
 ...
 */
router.delete('/reassureddelete-hero/:id', async (req, res) => {
  const { id } = req.params;
 ...
 */
router.put('/reassured-update-hero/:id', async (req, res) => {
  const { id } = req.params;
  const {
    title,
    subtitle,
 ...
 */
router.put('/reassured-update-hero-images/:id', upload.fields([
 ...
 */
router.get('/reassured-banners', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('reassured_banners')
      .select('*')
